...
iOSHybrid.listWebFrames = function (cb, exitCb) {
  ...
  var currentUrl;
  // For 9.2 we do not want to pass current URL as it is flaky and always returns "about: blank"
  if (parseFloat(this.iOSSDKVersion) !== 9.2) {
  ...
  if (this.remote !== null && this.args.bundleId !== null && this.remoteAppKey) {
    if (this.args.udid !== null) {
      ...
      this.remote.selectApp(this.remoteAppKey, currentUrl, function (err, perhapsModifiedAppKey, res) {
    ...
    if (this.args.udid !== null) {
      ...
      this.remote.connect(function (appDict) {
        ...
        this.remote.selectApp(appKey, currentUrl, function (err, perhapsModifiedAppKey, res) {
          if (err) return onDone(err);
          this.remoteAppKey = perhapsModifiedAppKey;
          onDone(null, res);
        }.bind(this), this.args.webviewConnectRetries);
